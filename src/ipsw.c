...
int ipsw_outside_file_extract_to_memory(const char *ipsw, const char *infile, unsigned char **pbuffer, size_t *psize, char* outside_path)
{
	size_t size = 0;
	unsigned char *buffer = NULL;

	FILE *f = fopen(outside_path, "rb");
	if (!f)
	{
		error("ERROR: %s: fopen failed for %s: %s\n", __func__, outside_path, strerror(errno));
		free(outside_path);
		return -2;
	}
	struct stat fst;
	...
	buffer[size] = '\0';
	printf("Extracted outside component at %s \n", outside_path);
	fclose(f);
	free(outside_path);
	*pbuffer = buffer;
	*psize = size;
