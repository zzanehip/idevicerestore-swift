...
static void usage(int argc, char *argv[], int err)
{
	...
	fprintf((err) ? stderr : stdout,
			...
			"\n"
			"Deca5 Required options:\n"
			" -K, --kernel-path     Specify kernelpath outside IPSW\n"
			" -D, --devicetree-path Specify devicetree outside IPSW\n"
			" -S, --ramdisk-path    Specify ramdisk outside IPSW\n"
			" -B, --ibec-path       Specify iBEC outside IPSW\n"
			"\n"
...
int idevicerestore_start(struct idevicerestore_client_t *client)
{
	...
	if (client->mode == &idevicerestore_modes[MODE_UNKNOWN] || (client->flags & FLAG_QUIT) || (client->mode != &idevicerestore_modes[MODE_DFU] && client->mode != &idevicerestore_modes[MODE_RECOVERY] && (client->flags & FLAG_LATEST_SHSH)))
	{
		mutex_unlock(&client->device_event_mutex);
	...
	else
	{
		...
		send_text("Extracting BuildManifest");
	...
	else if (client->mode->index == MODE_RECOVERY)
	{
		...
		if (recovery_send_ibec(client, build_identity, client->ibecpath) < 0)
		{
			mutex_unlock(&client->device_event_mutex);
...
int idevicerestore_boot(struct idevicerestore_client_t *client)
{
	int tss_enabled = 0;
	int result = 0;
...
void idevicerestore_client_free(struct idevicerestore_client_t *client)
{
	...
	if((client->ipsw) && !(client->flags & FLAG_BOOT))
	{
		free(client->ipsw);
	}
	if (client->version)
...
// 	while ((opt = getopt_long(argc, argv, "dhceswxtpli:u:nC:kyPRT:z:K:D:S:B:", longopts, &optindex)) > 0) {
// 		switch (opt) {
// 		case 'h':
// 			usage(argc, argv, 0);
// 			return 0;
...
// 		case 'K':
// 			if (!*optarg) {
// 				error("ERROR: Kernelcache path must not be empty!\n");
// 				usage(argc, argv, 1);
// 				return -1;
// 			}
// 			client->kernelpath = strdup(optarg);
// 			break;
// 		case 'D':
// 			if (!*optarg) {
// 				error("ERROR: Devicetree path must not be empty!\n");
// 				usage(argc, argv, 1);
// 				return -1;
// 			}
// 			client->devicetreepath = strdup(optarg);
// 			break;
// 		case 'S':
// 			if (!*optarg) {
// 				error("ERROR: Ramdisk path must not be empty!\n");
// 				usage(argc, argv, 1);
// 				return -1;
// 			}
// 			client->ramdiskpath = strdup(optarg);
// 			break;
// 		case 'B':
// 			if (!*optarg) {
// 				error("ERROR: iBEC path must not be empty!\n");
// 				usage(argc, argv, 1);
// 				return -1;
// 			}
// 			client->ibecpath = strdup(optarg);
// 			break;
...
{
	char *component_name = NULL;
	if (!path || !component_data || !component_size)
	{
		return -1;
	}
