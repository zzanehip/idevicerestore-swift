...
int recovery_enter_restore(struct idevicerestore_client_t* client, plist_t build_identity) {
	...
	if (recovery_send_devicetree(client, build_identity, client->devicetreepath) < 0) {
...
int recovery_send_outside_component(struct idevicerestore_client_t* client, plist_t build_identity, const char* component, const char* outside_path) {
	unsigned int size = 0;
	unsigned char* data = NULL;
	char* path = NULL;
	irecv_error_t err = 0;
	...
	unsigned char* component_data = NULL;
	size_t component_size = 0;
	int ret = extract_outside_component(client->ipsw, path, &component_data, &component_size, outside_path);
	free(path);
	if (ret < 0) {
		error("ERROR: Unable to extract component: %s\n", component);
		return -1;
...
int recovery_send_ibec(struct idevicerestore_client_t* client, plist_t build_identity, char* outside_path) {
...
int recovery_send_applelogo(struct idevicerestore_client_t* client, plist_t build_identity, char* outside_path) {
	...
	recovery_error = irecv_send_command(client->recovery->client, "bgcolor 180 101 74");
	if (recovery_error != IRECV_E_SUCCESS) {
...
int recovery_send_devicetree(struct idevicerestore_client_t* client, plist_t build_identity, char* outside_path) {
...
int recovery_send_ramdisk(struct idevicerestore_client_t* client, plist_t build_identity, char* outside_path) {
...
int recovery_send_kernelcache(struct idevicerestore_client_t* client, plist_t build_identity, char* outside_path) {
